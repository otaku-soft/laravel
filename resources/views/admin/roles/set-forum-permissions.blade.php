<div class ="font-medium text-gray-900 dark:text-gray-100">
    ...
    <ul id="sectionList" class="pb-3">
        @foreach ($sections as $section)
            <li class="pb-3 pt-3">
                <input type="checkbox" class="sectionElement" data-section="{{ $section->id }}" @if ($role->hasPermissionTo("section_".$section->id,"web")) checked @endif /> <b>{{ $section->name }} </b>
            </li>
            @foreach ($section->categories()->orderBy("order")->get() as $category)
                <li class="p-4 border">
                    <input type="checkbox" data-section="{{ $section->id }}"  data-id="{{ $category->id }}" class="categoryElement" @if ($role->hasPermissionTo("category_".$category->id,"web")) checked @endif /> {{ $category->name }}
                </li>
            @endforeach
        @endforeach
    </ul>
...
function savePermissions()
{
    ...
    $.post( "{{ route('roles_setForumPermissionsSave') }}", request)
        .done(function( data ) {
            window.location = "";
        });
}
